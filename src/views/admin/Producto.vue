<template>
    <div class="card">
        ...
        <DataTable ref="dt" :value="productos" :totalRecords="totalRecords" lazy :loading="loading" @page="onPage($event)"
            dataKey="id" :paginator="true" :rows="5"
            paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
            :rowsPerPageOptions="[3, 5, 10, 25]"
            currentPageReportTemplate="Mostrando {first} al {last} de {totalRecords} productos" responsiveLayout="scroll">
            <template #header>
            ...
            <Column field="nombre" header="NOMBRE" :sortable="true" headerStyle="width:21%; min-width:20rem;">
                <template #body="slotProps">
        ...
        <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="Producto" :modal="true" class="p-fluid">
            <!--{{ product }}-->
            <img :src="'demo/images/product/' + product.imagen" :alt="product.imagen" v-if="product.imagen" width="150"
            ...
            <div class="formgrid grid">
                <div class="field col">
                    ...
                    <InputNumber id="price" v-model="product.precio" mode="currency" currency="USD" locale="en-US"
                        :class="{ 'p-invalid': submitted && !product.precio }" :required="true" />
        ...
        <Dialog v-model:visible="productDialogImagen" :style="{ width: '650px' }" header="Imagen" :modal="true"
            class="p-fluid">
            <img :src="`http://127.0.0.1:8000/${product.imagen}`" alt="" width="200">
            <FileUpload customUpload @uploader="subirImagenProducto" :multiple="true" accept="image/*"
                :maxFileSize="1000000">
                <template #empty>
